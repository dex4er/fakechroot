...
These wrapper functions are in a shared library F<libfakechroot.so> which is
loaded through the C<LD_PRELOAD> mechanism of the dynamic loader. (See
ld.so(8))

In fake chroot you can install Debian bootstrap with debootstrap(8) command.
...
removed (eg. running F<gettext.sh> loads C<gettext> environment file). If
I<command> argument is fakeroot(1) this argument is ommited and next argument
...
fakechroot should not be run with real root priviliges. It might decrease the
...
A path to another dynamic linker (ie. F</lib/ld-linux.so.2> for i386
...
application won't work correctly, ie. busybox(1).
