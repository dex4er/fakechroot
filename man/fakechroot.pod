# pod2man --section=1 --release="Debian" --center=" " --date="7 Oct 2011"
#
=head1 NAME
...
Load additional confguration with environment.  The default environment type
is guessed based on command name.  If command is fakeroot(1) then next
argument is taken.

The configuration file name is C<I<type>.env> and can be located at
C<$HOME/.fakechroot> and C</etc/fakechroot> directories.

The default environment type is B<default> and its configuration file name is
C<default.env>.

=item B<-c>|B<--confdir> I<directory>
