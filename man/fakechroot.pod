# pod2man --section=1 --release="Debian" --center=" " --date="24 Oct 2005"
#
# $Id$
...
system dynamic linker (F</lib/ld-linux.so.2> for Linux) can not load
...
before chroot is called. It can be useful for accessing the real F</proc> and
F</dev> directory.
...
ldd(1) also doesn't work. You have to use wrapper. The example wrapper is
available at F<scripts/> directory in fakechroot's source package and it
is located at F</usr/share/doc/fakechroot/examples> directory.
