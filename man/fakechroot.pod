# pod2man --section=1 --release="Debian" --center=" " --date="5 Nov 2013"
#
...
A path to another dynamic linker (ie. C</lib/ld-linux.so.2> for i386
architecture, C</lib64/ld-linux-x86-64.so.2> for x86_64 architecture) if
...
C<LD_PRELOAD=libfakechroot.so>.  This library is loaded before the
...
are not compatible, try to use C<--use-system-libs> option.
...
de_dotdot function taken from mini_httpd - small HTTP server
