...
# _AH_CHECK_FUNC_ARGTYPES(FUNCTION-NAME, TYPES-RETURN, [TYPES-ARG1, TYPES_ARG2, ...])
# -----------------------------------------------------------------------------------
...
# AC_CHECK_FUNC_ARGTYPES(FUNCTION-NAME, PROLOGUE, TYPES-DEFAULT, TYPES-RETURN, TYPES-ARG1, TYPES-ARG2, ...])
# ----------------------------------------------------------------------------------------------------------
...
AC_DEFUN([AC_CHECK_FUNC_ARGTYPES],
    [_AH_CHECK_FUNC_ARGTYPES([$1], m4_shift3($@))
        AC_CACHE_CHECK([types of arguments for $1],
            [ac_cv_func_$1_args],
                [m4_join([ ], [for ac_return in], m4_map([_AC_CHECK_FUNC_ARGTYPES_QUOTE], [$4]), [; do])
                    m4_define([myarglist])
                    m4_define([myacarglist])
                    m4_define([myfuncarglist])
                    m4_for([myargn], [5], m4_count($@), [1],
                        [m4_define([myacvar], m4_join([], [ac_arg], m4_eval(myargn - 4)))
                            m4_define([myacarglist], m4_make_list(myacarglist, myacvar))
                            m4_define([myarglist], m4_make_list(myarglist, m4_join([], myacvar, [(arg], m4_eval(myargn - 4), [)])))
                    ...
                    AC_COMPILE_IFELSE(
                        [AC_LANG_PROGRAM(
                                ...
                                [m4_for([myargn], [5], m4_count($@), [1],
                                    [m4_define([myacvar], m4_join([], [ac_arg], m4_eval(myargn - 4)))
                                        m4_define([myargvar], m4_join([], [arg], m4_eval(myargn - 4)))
                                        m4_join([], [#define ],
                                            myacvar, [(_) $], myacvar
                                            )])]
                            ...
                            [m4_define([myaccvargs], m4_if(m4_cmp(m4_count(myacarglist), 0), [1],
                                ...
                                m4_join([], [ac_cv_func_$1_args="$ac_return], myaccvargs, ["; break ], m4_eval(m4_count($@) - 3))])
                    m4_for([myargn], [4], m4_count($@), [1], m4_echo([done; ]))
                    m4_join([], [: ${ac_cv_func_$1_args='(default) ], m4_join([;], $3), ['}])])
        m4_echo([
            ac_save_IFS=$IFS; IFS=';'
